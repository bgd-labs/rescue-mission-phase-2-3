...
contract V2AvaPoolTest is Test {
  ...
  function testUpdateV2EthPool() public {
    assertEq(LendingPool(address(AaveV2Avalanche.POOL)).LENDINGPOOL_REVISION(), 2);
    _updatePool();
    assertEq(LendingPool(address(AaveV2Avalanche.POOL)).LENDINGPOOL_REVISION(), 3);
  }
  ...
  function testRescueUsdcE() public {
    ...
    assertEq(IERC20(USDCe_TOKEN).balanceOf(MERKLE_DISTRIBUTOR), USDC_E_RESCUE_AMOUNT);
    vm.stopPrank();
  }
  ...
  function testRescueUsdtE() public {
    ...
    assertEq(IERC20(USDTe_TOKEN).balanceOf(MERKLE_DISTRIBUTOR), USDT_E_RESCUE_AMOUNT);
    vm.stopPrank();
  }
