...
git-diff-storage :
	@mkdir -p diffs
	@printf '%s\n%s\n%s\n' "\`\`\`diff" "$$(git diff --no-index --diff-algorithm=patience --word-diff=porcelain --ignore-space-at-eol ${before} ${after})" "\`\`\`" > diffs/${out}.md
...
storage-diff:
	...
	make git-diff-storage before=reports/v1Pool_layout.md after=reports/rescue_v1Pool_layout.md out=v1Pool_layout_diff
	...
	make git-diff-storage before=reports/v2AmmPool_layout.md after=reports/rescue_v2AmmPool_layout.md out=v2AmmPool_layout_diff
	...
	make git-diff-storage before=reports/v2EthPool_layout.md after=reports/rescue_v2EthPool_layout.md out=v2EthPool_layout_diff
	...
	make git-diff-storage before=reports/v2PolPool_layout.md after=reports/rescue_v2PolPool_layout.md out=v2PolPool_layout_diff
	...
	make git-diff-storage before=reports/v2AvaPool_layout.md after=reports/rescue_v2AvaPool_layout.md out=v2AvaPool_layout_diff
	...
	make git-diff-storage before=reports/v2EthAToken_layout.md after=reports/rescue_v2EthAToken_layout.md out=v2EthAToken_layout_diff
	...
	make git-diff-storage before=reports/v2PolAToken_layout.md after=reports/rescue_v2PolAToken_layout.md out=v2PolAToken_layout_diff
